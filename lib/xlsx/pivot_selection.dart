...
class PivotSelection with XmlNodeWrapper {
  ...
  PivotArea? get references{
    var _node = node.into(selector: (c)=>c.type == XmlElementType.Normal && c.name.removeNamespace() == "pivotArea");
    if(_node != null){
      return PivotArea(file,_node);
    }
    return null;
  }
}
...
class PivotArea with XmlNodeWrapper {
  ...
  PivotAreaReferences? get references{
    var _node = node.into(selector: (c)=>c.type == XmlElementType.Normal && c.name.removeNamespace() == "references");
    if(_node != null){
      return PivotAreaReferences(file,_node);
    }
    return null;
  }
}
...
class PivotAreaReferencesIterator extends XmlChildNodeIterator<PivotAreaReference>{
  ...
  @override
  bool selector(n)=>n.type == XmlElementType.Normal && n.name.removeNamespace() == "reference";
}
...
class PivotAreaReferenceIterator extends XmlChildNodeIterator<IndexType>{
  ...
  @override
  bool selector(n)=>n.type == XmlElementType.Normal && n.name.removeNamespace() == "x";
}
